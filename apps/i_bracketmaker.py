...
def app():
    ...
    pfs = LRF.predict(r1p[xcol]) + np.random.rand(32)*10-3
    ...
    pfs = LRF.predict(BBstats[xcol]) + np.random.rand(16)*10-3
    ...
    pfs = LRF.predict(BBstats[xcol]) + np.random.rand(8)*10-3
    pus = RFU.predict(BBstats[xcol])  
    for x in range(49,57):
    ...
    pfs = LRF.predict(BBstats[xcol]) + np.random.rand(4)*10-3
    pus = RFU.predict(BBstats[xcol])  
    for x in range(57,61):
    ...
    pfs = LRF.predict(BBstats[xcol]) + np.random.rand(2)*10-3
    pus = RFU.predict(BBstats[xcol])  
    for x in range(61,63):
    ...
    pfs = LRF.predict(BBstats[xcol]) + np.random.rand()*10-3
    pus = RFU.predict(BBstats[xcol])  
    for x in range(63,64):
