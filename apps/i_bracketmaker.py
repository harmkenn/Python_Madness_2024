...
def app():
    ...
    pfs = LRF.predict(r1p[xcol]) + np.random.rand(32)*0-0
    ...
    pfs = LRF.predict(BBstats[xcol]) + np.random.rand(16)*0-0
    ...
    pfs = LRF.predict(BBstats[xcol]) + np.random.rand(8)*0-0
    pus = RFU.predict(BBstats[xcol])  
    for x in range(49,57):
    ...
    pfs = LRF.predict(BBstats[xcol]) + np.random.rand(4)*0-0
    pus = RFU.predict(BBstats[xcol])  
    for x in range(57,61):
    ...
    pfs = LRF.predict(BBstats[xcol]) + np.random.rand(2)*0-0
    pus = RFU.predict(BBstats[xcol])  
    for x in range(61,63):
    ...
    pfs = LRF.predict(BBstats[xcol]) + np.random.rand()*0-0
    pus = RFU.predict(BBstats[xcol])  
    for x in range(63,64):
